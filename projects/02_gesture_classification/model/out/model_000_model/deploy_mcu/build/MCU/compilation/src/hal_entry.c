...
void hal_entry(void)
{
    ...
    for (int n = 0; n < ntimes; n += 1) {
        memcpy(GetModelInputPtr_input(), model_input, model_input_SIZE);
    ...
    // Comparing outputs
    {
        ...
        int misses = CompareOutput(model_output_70010, GetModelOutputPtr_output_70010(), model_output_70010_COUNT, 0.1f);
        if (misses == 0) {
            SEGGER_RTT_WriteString(0, "MATCH\r\n");
        } else {
            SEGGER_RTT_printf(0, "FAILED: %d/%d misses\r\n", misses, model_output_70010_COUNT);
        }
    }
