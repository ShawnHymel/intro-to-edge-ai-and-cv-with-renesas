...
#include "fsp_custom/mipicsi_vin_hal_driver.h"
#include "fsp_custom/mipicsi_vin_hal_driver_config.h"
...
/* Used by ISR to signal that a new camera frame is ready */
...
void camera_capture_post_process(void)
{
    ...
    // Ensure invalidate completes before reading
    __DSB();
#endif
...
void g_cam_mipicsi_vin_user_callback(vin_event_t vin_event, mipicsi_event_status_t mipicsi_event)
{
    /* Bare metal version: set global flag */
