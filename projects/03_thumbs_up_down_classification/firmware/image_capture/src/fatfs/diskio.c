...
DRESULT disk_write(BYTE pdrv, const BYTE *buff, LBA_t sector, UINT count)
{
    ...
    /* Write sectors one at a time using CMD24 (single block write) */
    static BYTE rx_dummy[512];

    while (count) {
        cmd_res = send_cmd(CMD24, sector);
        if (cmd_res != 0) {
            DESELECT();
            break;
        }
        ...
        /* Check response */
        write_res = spi_txrx(0xFF);
        if ((write_res & 0x1F) != 0x05) {
            DESELECT();
            break;  /* Write failed */
        }
        ...
        if (CardType & CT_BLOCK) {
            sector++;  /* Block addressing: increment by 1 */
        } else {
            sector += 512;  /* Byte addressing: increment by 512 */
        }
        count--;
    }
