...
/***********************************************************************************************************************
 * IMPORTANT: This is a modified/older version of rm_ethosu.c that MUST be used instead of the FSP-generated version.
 *
 * ISSUE: FSP 6.3.1's rm_ethosu.c has a bug where it derives secure_enable and privilege_enable from
 *        BSP_CFG_OPTION_SETTING_OFS2 at compile time, ignoring the values in rm_ethosu_cfg_t.
 *        This causes NPU inference to fail when OFS2 is configured for Non-Secure/Unprivileged
 *        (the default for non-TrustZone projects), because the NPU needs Secure/Privileged memory
 *        access attributes regardless of how the NPU peripheral itself is accessed.
 *
 * FIX: This version reads secure_enable and privilege_enable from the config struct (p_cfg->),
 *      allowing the application to explicitly set these values (typically both to 1).
 *
 * USAGE:
 *   1. Exclude the FSP-generated rm_ethosu.c from the build:
 *      - Right-click: ra/fsp/src/rm_ethosu/rm_ethosu.c
 *      - Select: Resource Configurations -> Exclude from Build...
 *      - Check all build configurations (Debug, Release)
 *
 *   2. In your application code, set secure_enable=1 and privilege_enable=1 before calling RM_ETHOSU_Open():
 *
 *      rm_ethosu_cfg_t ethosu_cfg = g_rm_ethosu0_cfg;
 *      ethosu_cfg.secure_enable = 1;
 *      ethosu_cfg.privilege_enable = 1;
 *      RM_ETHOSU_Open(&g_rm_ethosu0_ctrl, &ethosu_cfg);
 *
 * SOURCE: Derived from vision_ai_ethosu_mipicsi_glcd_ek_ra8p1_llvm_mera_mobilenet_v1 example project.
 *
 * BUG REPORT: https://community.renesas.com/mcu/ra/f/forum/58272/bug-report-fsp-6-3-1-rm_ethosu-c-incorrectly-derives-secure_enable-privilege_enable-from-ofs2
 **********************************************************************************************************************/
